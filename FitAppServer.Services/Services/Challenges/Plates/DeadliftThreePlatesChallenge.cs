...
public class DeadliftThreePlatesChallenge : IChallenge
{
    private readonly FitAppContext _context;

    public DeadliftThreePlatesChallenge(FitAppContext context)
    {
        _context = context;
    }
    ...
    public async Task Check(WorkoutAction action, Workout workout)
    {
        var correctSet = workout.Exercises
            .Where(q => q.ExerciseInfoId == (int)WorkoutTypeCode.Deadlift)
            .SelectMany(q => q.Sets)
            .FirstOrDefault(q => q.Weight >= 140);
    ...
    public string GetId() => "deadliftThreePlatesChallenge";
